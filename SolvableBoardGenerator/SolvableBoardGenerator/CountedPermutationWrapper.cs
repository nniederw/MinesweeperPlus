...
{
    public class CountedPermutationWrapper
    {
        public BitArray Permutation;
        public uint MineCount;
        public IReadOnlyDictionary<(int x, int y), int> IndexLookupTable;
        public CountedPermutationWrapper(BitArray permutation, uint mineCount, IReadOnlyDictionary<(int x, int y), int> indexLookupTable)
        {
        ...
        public IEnumerable<(int x, int y)> SharedPositions(CountedPermutationWrapper other)
            => IndexLookupTable.KeyIntersection(other.IndexLookupTable);
        public bool Intersectable(CountedPermutationWrapper other, IEnumerable<(int x, int y)> sharedPositions)
        {
        ...
        public CountedPermutationWrapper Intersect(CountedPermutationWrapper other, IReadOnlyDictionary<(int x, int y), int> newIndexLookupTable, IEnumerable<(int x, int y)> sharedPos)
        {
            ...
            uint newMineCount = MineCount + other.MineCount - (uint)sharedPos.Count(i => At(i));
            return new CountedPermutationWrapper(NewBArr, newMineCount, newIndexLookupTable);
        }
    }
}