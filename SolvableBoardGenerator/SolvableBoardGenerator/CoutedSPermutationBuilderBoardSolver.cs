...
{
    public class CoutedSPermutationBuilderBoardSolver : BaseBoardSolver
    {
        ...
        public CoutedSPermutationBuilderBoardSolver() : base() { }
        public CoutedSPermutationBuilderBoardSolver(IBoard board, bool verboseLogging = false) : base(board, verboseLogging) { }
        ...
        private bool PermutationBuildingAlgorithm()
        {
            ...
            LastActivePermutationNodes = activeRegions;
        ...
        {
            uint FloatingSquares = (uint)Board.AllSquares().Count(i => !IsOpenedSquare(i) && !ActiveUnopenedSquares.Contains(i));
            uint trivialMin = uint.MaxValue;
            uint trivialMax = 0;
            var groups = new List<List<MineRegionPermutationNode>>();
            {
                var nodes = LastActivePermutationNodes.ToHashSet();
                foreach (var reg in LastActivePermutationNodes)
                {
        ...
        private (uint trivialMinMineCount, uint trivialMaxMineCount) CalculateTrivialCounts(List<MineRegionPermutationNode> connectedNodes)
        {
            if (connectedNodes.Count == 1)
            {
                var n = connectedNodes.Single().MineRegionPermutation;
